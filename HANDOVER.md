...
### 2.0.1 에서 급히 고친 것 — 안드로이드에서 한 번도 안 돌았다

2.0.0 을 내고 몇 시간 뒤 코드를 다시 읽다 알았다. 통로만 갈아 끼웠고
**돌아도 되는가를 묻는 자리는 안 고쳤다.** 안드로이드에서 셋이 겹쳤다.

- 설정의 '동기화' 칸이 `ICloudSync.supported` 로 가려져 아예 안 보였다.
  구글 드라이브를 붙여 놓고 그걸 고를 화면을 막아 놨다.
- 보였더라도 `syncNow()` 가 첫 줄에서 그냥 돌아갔다.
- 뿌리 경로를 애플 채널에 물었다. 안드로이드에서는 아무것도 안 돌아온다.

**한 판단이 세 군데로 흩어져 있으면 반드시 한 군데를 빠뜨린다.** 이제
`_viaApple` 한 줄만 본다. 창고가 또 늘면 거기만 고친다.

이름도 정리했다. `supported` 는 '아이클라우드를 쓸 수 있는 기기인가'라는
원래 뜻으로 남기고, '지금 고른 창고로 오갈 수 있는가'는 `active` 로 뺐다.
**이름이 실제보다 넓으면 다음 사람이 그 넓이를 믿는다.**

거짓 초록불도 하나 없앴다. `_run()` 이 `ensureDir` 의 답을 안 보고 지나가서,
구글 로그인이 안 된 판에서도 끝까지 내려가 화면이 '맞춰 놨다'고 말했다.
토큰이 없어 빈손으로 오는 것과 '아무것도 없는 창고'를 코드는 구별 못 한다.
이제 던지고 '꺼짐'으로 둔다. **거짓 초록불보다 정직한 회색불이 낫다.**
...
## 8-7. 심사 상태는 브라우저로 보지 않는다 — 2026-08-20 새벽

`python3 tool/review_status.py` 한 줄이면 두 앱의 심사 상태가 사람 말로 나온다.
`skyblue` 나 `long` 을 붙이면 하나만 본다.

### 왜 만들었나

그 전까지 심사 확인은 크롬에 열어 둔 App Store Connect 탭을 읽어서 했다.
문제는 **그 탭이 사장님도 쓰는 탭**이라는 것이었다. 사장님이 다른 화면으로
옮겨 두면 뺏을 수 없다(그러라고 정해 둔 규칙이다). 2026-08-19 밤에는 그래서
열두 번의 감시가 연달아 건너뛰어졌다 — 한 번도 "못 봤다"고 티가 나지 않은 채로.

**사람과 도구가 같은 창을 나눠 쓰면 결국 도구가 눈을 감는다.** 그리고 눈을
감았다는 사실조차 조용하다. 이 모양이 보이면 창을 나누지 말고 통로를 따로 낼 것.

### 열쇠

`~/.appstoreconnect/asc.py` 의 `api()` 를 쓴다. 열쇠는 거기 있고 **저장소에는
안 들어온다.** `tool/review_status.py` 에는 열쇠가 없다 — 어디서 찾는지만 있다.
KEY_ID·ISSUER 는 어디에도 찍지 말 것.

### 애플이 말해 주지 않는 것

거절 사유의 **본문은 API 로 안 온다.** 상태가 `REJECTED` /
`METADATA_REJECTED` 면 도구가 그렇게 알려 준다. 사유는 App Store Connect 의
리졸루션 센터를 열어야 보인다 — 그때는 사람이 봐야 한다.

### 2026-08-20 03:20 시점

- Skyblue Note 1.0 — 냈다, 줄 서 있다(빌드 42, 올린 지 2일 12시간).
- Long Time, Easy Life 1.9.2 — 심사 중(빌드 46, 18시간 전).

새 앱의 첫 심사는 이미 나가 있는 앱의 판올림보다 줄이 길다. 나중에 낸 1.9.2 가
먼저 심사에 들어간 것은 그 때문으로 보인다. 이상 신호는 아니다.
