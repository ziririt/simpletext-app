...
## 이 세션의 이름은 `1번-스카이블루` 다 (2026-09-12)
...
2026-09-12 부터 여섯 저장소가 **공용 git 훅**을 같이 쓴다(`core.hooksPath` → `~/Developer/_ops/hooks`).
잠금을 쥔 사람 말고는 push 가 거부되고, 커밋에 열쇠가 섞이면 커밋 자체가 막힌다.

**이름을 새로 지어내지 마라.** 계정 이름으로는 서로를 구분할 수 없다 —
같은 계정에서 세션이 둘 도는 것이 일상이고, 2026-09-12 충돌의 실제 원인이 그것이었다.
이름이 갈리면 **내가 잡은 잠금을 내가 못 푸는** 일이 생긴다.

큰 작업은 `~/Developer/_ops/bin/lock.sh claim simpletext_app "무엇을 한다"` 로 잠그고,
끝나면 `release` 한다. 잠금이 없으면 아무것도 막지 않는다(4시간 뒤 저절로 무효, 훅이 깨지면 통과).
...
- 소유자 확인을 기다리는 것은 `PENDING.md` 에 남긴다. 이어받는 AI 가
  첫 답변에서 그것부터 내민다
...
> 파일에 남긴다. 이 저장소의 검사기들(handoff_check·pending_check)은
> 전부 그 한 문장에서 나왔다.
...
## allow 목록을 늘리지 않는 것 자체가 방어다 (2026-09-13, 주간 수집 8회차)

- 이 저장소의 `.claude/settings.json` 은 **deny 전용**이다. 서명 키와 스토어·플랫폼
  자격증명 파일의 읽기·쓰기를 막고, 강제 푸시·`reset --hard`·`git add -A`·
  `git clean -fd` 를 명령 차원에서 막는다. 막는 대상의 실제 목록은 그 파일이 갖는다.
  **allow 목록은 한 줄도 없다. 앞으로도 넣지 않는다.**
- 왜 이 규칙이 필요한가: 2026-09-10 CLI 2.1.268·2.1.269에서 권한 우회 결함 세 건이
  한꺼번에 고쳐졌는데, 셋 중 둘이 "allow 규칙이 생각보다 넓게 적용되더라" 형태였다.
  `Bash(tee:*)` allow 하나가 작업 폴더 **밖** 목적지까지 덮었고, macOS 심볼릭 링크
  경로(`/tmp`·`/var`)에 건 규칙은 같은 파일을 실제 위치로 지정하면 비껴갔다.
  우리가 allow를 안 쓴 덕분에 그중 하나는 애초에 열려 있지도 않았다.
- 그래서: 편의를 위해 `Bash(...)` allow를 한 줄 넣고 싶어지는 순간이 반드시 온다.
  그때 넣지 않는다. 도구가 매번 묻는 쪽이, 규칙이 조용히 안 걸리는 쪽보다 낫다.
- 덧붙여 **deny 규칙은 CLI 버전에 종속된 방어다.** 같은 규칙 파일이 판올림 없이는
  한 달 뒤에 덜 막는다. 터미널 CLI는 데스크톱 앱과 달리 수동 갱신이므로
  `claude --version` 확인은 편의가 아니라 보안 절차다. 주간 점검 항목으로 둔다.

## Dart MCP 서버는 관문 앞의 연장이지 관문이 아니다 (2026-09-20, 주간 수집 9회차)

이 저장소 뿌리에 `.mcp.json` 이 있다. Flutter 팀이 공식으로 내는 **Dart·Flutter MCP 서버**를
붙여 둔 것이고, Dart SDK에 이미 들어 있어 따로 설치할 것은 없다(`dart mcp-server`).
붙이면 에이전트가 **살아 있는 분석기 진단·심볼 해석·시험 실행기·의존성 정보**에 직접 닿는다.

**그래도 푸시의 관문은 `tool/verify.sh` 하나뿐이다.** 순서를 못 박아 둔다.

1. 고친다
2. MCP 서버로 원인을 좁힌다 (분석기 진단·시험 실행)
3. **`tool/verify.sh` 를 돌려 "전부 통과"를 확인한다**
4. 푸시한다

3번을 건너뛰지 않는다. MCP 서버가 "분석기에 문제 없다"고 답하는 것과
`tool/verify.sh` 14종이 전부 통과하는 것은 다른 말이다 — 인수인계서 신선도,
다국어 정합성, 버전 표기, 스토어 등록정보는 분석기가 보지 않는다.
**"MCP가 괜찮다고 했으니까"로 관문을 건너뛰는 날이 오면 그날 관문은 꺼진다.**

도구는 필요한 것만 연다. `.mcp.json` 의 `--disable` 로 앱을 띄워 놓고 쓰는 갈래
(`flutter_driver`·`flutter_app_lifecycle`)와 `create_project`·`pub_dev_search` 를 꺼 두었다.
자동 검증 흐름에서 쓸 일이 없고, 켜 두면 도구 수만 늘어 다이어트 기준
(켠 MCP 10개 미만 · 활성 도구 80개 미만)을 잠식한다. **연장통을 통째로 여는 게 아니라
쓸 연장만 꺼내 두는 것이 다이어트다.**

전제: `dart` 가 PATH에 있어야 한다. 이 저장소는 공개 저장소라 `.mcp.json` 에
개인 폴더 경로를 적지 않고 `"command": "dart"` 로 두었다. Flutter bin을 PATH에
넣어 두지 않은 환경에서는 MCP 서버만 연결에 실패하고 나머지는 그대로 동작한다.

공식 스킬 10종(`flutter/agent-plugins`)은 **설치하지 않는다.** 절반이 프로젝트를
처음 세울 때 쓰는 조리법이고, 이 저장소는 다섯 판을 굴리는 성숙한 코드베이스다.
카탈로그는 **메뉴로만 본다** — 구체적 필요가 생겼을 때 그 하나만 꺼내 읽는다.

원문: docs.flutter.dev/ai/get-started · github.com/flutter/agent-plugins
