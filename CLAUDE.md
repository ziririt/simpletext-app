...
- **작업할 때마다 버전을 올린다 (소유자 요청 2026-08-12).** 올리기만 하면 소용없고
  화면에서 보여야 한다 — 앱은 설정 맨 위 오른쪽에 `ver.1.7 (245)` 형식으로.
  **2026-09-12 개편 — 소유자 지시 "앱스토어 기준으로 바꿔라".**
  사람이 손대는 값은 `lib/version.dart` 안의 둘뿐이다.
    · `appVersion` — **앱스토어에 보이는 그 이름**(1.7). 새 판을 낼 때만 올린다
    · `appBuild` — 빌드 번호. 작업할 때마다 올린다. 절대 안 내린다
  `pubspec.yaml` 의 `version:` 은 **사람이 정하지 않는다.** `3.<빌드>.0+<빌드>`
  꼴로 따라 적는 기계용 값이다 — 애플이 꾸러미 버전을 못 내리게 해서
  (ITMS-90062) 생긴 자리이고, 어느 화면에도 안 나온다. 까닭은
  `lib/version.dart` 머리말에 있다. 어긋나면 `tool/version_check.py` 가 잡는다.
  스토어에 낼 때 `appVersion` 을 안 올리면 `tool/appstore_ios.sh` 가 굽기 전에
  애플에게 물어보고 멈춘다.
