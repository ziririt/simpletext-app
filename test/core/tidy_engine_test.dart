...
| 2023년 3~4월 | 회사명을 리디북스에서 리디로 변경했습니다. | IP 사업 확장 전략을 밝힌 단계입니다. |''';
  ...
  // 2026-08-12 재현 fixture — 실제 사용 중 발견한 사고.
  // ChatGPT 앱에서 렌더된 표를 복사하면 마크다운 파이프가 아니라 '탭'으로 넘어온다.
  ...
  group('탭으로 구분된 표 (앱에서 복사한 경우 · 2026-08-12)', () {
    ...
    test('탭 표도 스프레드시트로 옮길 수 있다', () {
      final t = extractTables(tsvIn).tables;
      expect(t.length, 1, reason: '탭 표를 표로 찾지 못했다');
      expect(t.first.header, ['역할', '예시 비중']);
      expect(t.first.rows.length, 3);
    });

    test('두 번 정리해도 결과가 그대로다', () {
      final once = tidy(tsvIn, aiOpts()).text;
      expect(tidy(once, aiOpts()).text, once);
    });

    test('탭이 한 줄뿐이면 표로 보지 않는다 (들여쓰기 오탐 방지)', () {
      expect(extractTables('제목\n\t들여쓴 문장 하나').tables, isEmpty);
    });

    test('탭 없는 줄글은 표로 보지 않는다', () {
      expect(extractTables('오늘은 날씨가 좋다\n내일은 비가 온다').tables, isEmpty);
    });
  });
...
[3] 리디, 책의 날 맞아 디지털 굿즈 배포 https://ridicorp.com/2021/04/2021-world-book-day-digital-goods/''';
  ...
  // 2026-08-14 소유자 요청: "'2026-08-03(월) 13:58 KST'과 같은 출력시간도 삭제.
  // 제목은 ... 이런 시각 표시 말고 문서의 맨 위의 텍스트 한줄을 자동으로 제목으로."
  ...
  group('출력 시각 머리글 제거 (2026-08-14)', () {
    ...
    test('여러 형식의 시각 줄을 지운다', () {
      const stamps = [
        '2026-08-03(월) 13:58 KST',
        '2026-08-03 13:58',
        '2026. 8. 3. 오후 1:58',
        '2026년 8월 3일 (월) 13:58 KST',
        '2026/08/03 13:58:20',
        '2026-08-03(Mon) 13:58 KST',
        '출력 시각: 2026-08-03 13:58 KST',
        '기준 시각 — 2026-08-03(월)',
        '작성일: 2026년 8월 3일',
    ...
    test('날짜가 든 본문·소제목은 절대 지우지 않는다', () {
      // 여기를 잘못 잡으면 사용자가 쓴 소제목이 통째로 사라진다.
      const keeps = [
        '2024~2025년',
        '2026년 7월 공지, 8월 20일 적용 예정',
        '2026년 8월 3일', // 시:분이 없으면 그냥 날짜 소제목이다
        '2026-08-03 회의록',
        '13:58에 만나기로 했다',
      ];
      for (final k in keeps) {
        expect(firstLineOf('$k$body'), k, reason: '"$k"를 지워 버렸다');
      }
    });
    ...
    test('두 번 정리해도 결과가 같다', () {
      const raw = '2026-08-03(월) 13:58 KST\n\n본문 첫 줄입니다.';
      final once = tidy(raw, aiOpts()).text;
      expect(tidy(once, aiOpts()).text, once);
    });

    test('AI 서두 제거를 끄면 시각 줄도 남는다', () {
      const raw = '2026-08-03(월) 13:58 KST\n\n본문 첫 줄입니다.';
