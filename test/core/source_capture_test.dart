...
void main() {
  group('주소', () {
    test('다섯 서비스 주소를 안다', () {
      ...
      expect(
        sourceFromCapture('https://gemini.google.com/app/x').name,
        kGemini,
      );
      expect(
        sourceFromCapture('https://www.perplexity.ai/search/x').name,
        kPerplexity,
      );
    ...
    test('링크에 붙어 오는 utm 표식도 잡는다', () {
      expect(
        sourceFromCapture('https://example.com/a?utm_source=chatgpt.com').name,
        kChatGpt,
      );
    });
  ...
  group('HTML 조각의 표식', () {
    ...
    test('제미나이의 응답 칸 — id', () {
      expect(
        sourceFromCapture(
          '<div id="model-response-message-contentr_82cc">',
        ).name,
        kGemini,
      );
    });
  ...
  group('안 찍는 자리', () {
    ...
    test('두 서비스가 함께 보이면 아무 말도 안 한다', () {
      ...
      expect(
        sourceFromCapture(
          'https://chatgpt.com/c/1 https://claude.ai/chat/2',
        ).isKnown,
        isFalse,
      );
    });
    ...
    test('테일윈드 기본 클래스에는 안 걸린다 — prose 는 챗지피티에도 퍼플렉시티에도 있다', () {
      expect(
        sourceFromCapture('<div class="prose dark:prose-invert">').isKnown,
        isFalse,
      );
    });
  });
