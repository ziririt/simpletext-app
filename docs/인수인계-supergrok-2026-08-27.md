...
- **ezlong.com 애드센스**: '가치가 별로 없는 콘텐츠'로 **거절 상태**. 도구
  ...
  고친 뒤에 눌러야 한다. **이건 폐기가 아니라 보류다** — §12의 '웹앱 광고'를
  읽어라.
...
### 웹앱 광고(애드센스) — **보류. 폐기가 아니다.**

2026-08-27 소유자 확인: "접을 수는 없다. 나중에 다시 꺼내야 한다."
그러니 다음 파트너는 이 항목을 지우지 말고, 아래 조건이 갖춰지는 날 다시 꺼내라.

- **왜 하려는가** — 소유자 구상은 "웹앱은 광고 버전으로 두되, 아이폰이든
  안드로이드든 하나라도 결제한 사람이 웹앱에 오면 배너를 없앤다"다. 결제자
  판별은 이미 된다(드라이브 계정 파일의 `ent`). **광고 쪽만 없다.**
- **지금 왜 안 되는가** — 두 가지가 겹쳐 있다.
  - 우리 앱의 광고는 안드로이드·아이폰에서만 나간다
    (`adsSupported = !kIsWeb && (Android || iOS)`). 웹·맥·윈도우에는 광고가
    아예 없다. AdMob 플러터 꾸러미(`google_mobile_ads`)가 두 platform만
    선언하기 때문이고, 이건 우리가 어떻게 할 수 있는 부분이 아니다.
  - 웹은 AdMob이 아니라 **애드센스** 자리인데, ezlong.com이 '가치가 별로 없는
    콘텐츠'로 거절돼 있다(§8). 그 해결이 선행 조건이다.
- **다시 꺼낼 조건** — ezlong.com 콘텐츠 보강 → 애드센스 재검토 통과.
  그 전에는 손대 봐야 심을 곳이 없다.
- **그때 붙이는 방법** — 웹 빌드에만 애드센스 스크립트를 넣고, 로그인한
  결제자에게는 숨긴다. 판정은 새로 만들지 말고 `premiumHere(e, kFamilyWeb, now)`
  를 그대로 쓴다. 로그인하지 않은 사람은 우리가 누군지 알 수 없으므로 광고가
  나간다 — 그게 정상이고, "웹은 광고 버전"과 어긋나지 않는다.
- **하지 말 것** — 맥이나 윈도우 앱 안에 웹뷰를 심고 거기에 애드센스를 띄우는
  편법. 구글은 앱 안의 광고를 AdMob 쪽으로 몰아 왔고, 이 자리는 정책이 명확하지
  않다. **애드센스 계정이 정지되면 ezlong.com 전체가 같이 죽는다.** 맥에 광고를
  넣고 싶으면 자체 하우스 배너(우리 다른 앱 홍보)가 안전한 길이다.
- **급하지 않은 이유도 적어 둔다** — 웹에서도 하루 한도(정리 3회·마법사 2회)가
  걸리므로, 광고가 없어도 무료와 유료의 차이는 이미 서 있다. 즉 웹 광고는
  수익 항목이지 유료화의 전제가 아니다. 순서를 뒤집지 마라.
