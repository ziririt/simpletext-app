...
///
/// 가 돌려주는 version 은 **스토어 페이지에 보이는 이름**(1.7)이다.
///
/// 2026-09-11 에는 앱 안 이름이 3.17.x 라 이것과 견줄 수가 없었다. 그냥
/// 견주면 3.17.18 > 1.6 이라 **영원히 "최신입니다"** 라고 답하는 앱이 됐다.
/// 그때는 `kStoreVersion` 이라는 쪽지를 하나 더 들려서 막았다.
///
/// **2026-09-12 부터는 쪽지가 필요 없다.** 소유자 지시로 앱 안 이름 자체를
/// 앱스토어 이름으로 바꿨다(version.dart). `appVersion` 이 곧 '1.7' 이라
/// 애플이 주는 값과 같은 자로 잰다.
///
