...
const List<Paper> kPapers = [
  ...
  // 서리. 2026-08-19에 '모눈'에서 이름을 바꿨다 — 세로줄을 뺀 뒤로는
  // 모눈이 아니었다. 이름이 화면을 설명하지 않고 화면의 과거를 설명하고
  // 있었다.
  //
  // '서리'인 까닭: 이 종이의 핵심은 차가운 흰빛이고(몰스킨의 아이보리와
  // 갈라지는 지점이 거기다), 서리는 유리에 가는 결을 남긴다. 격자를
  // 뜻하는 말이 아니라 흔한 자연어라 아홉 나라 말로 옮겨도 안 어색하다.
  //
  // 2026-08-18 소유자 지시로 세로줄을 뺐다 — "원고지를 뺐던 이유도
  ...
  // 이 넷은 '줄 없는 색 종이'다. 줄이 필요한 사람은 위의 몰스킨·서리를
...
const Map<String, String> kRetiredPapers = {
  'manuscript': 'frost',
  // 2026-08-19에 '모눈'을 '서리'로 고쳐 부르면서 id 도 바꿨다. 이미 그걸
  // 고른 기기가 있고, 그 기기의 설정에는 아직 옛 이름이 적혀 있다.
  'grid': 'frost',
};
