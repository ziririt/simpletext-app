...
- 현재 버전: **3.17.10+232** (`pubspec.yaml`, `lib/version.dart`)
...
- **숫자 딱지** — 굴리는 동안만 뜬다. `62%`. 멈추고 1.4초 더 남긴다 —
  굴리기를 멈춘 그 순간이 바로 '지금 어디지'를 묻는 순간이라, 그때 숫자가
  없으면 아무 쓸모가 없다.

한 화면 반이 안 되는 글에는 아예 안 나온다(`_worth`). 글 쓰는 화면에
장식은 두지 않는다.
...
- 메모를 열면 **바로 책갈피 자리로 데려간다.** 전환이 끝나면 머리 밑에
  '책갈피 62%에서 이어 읽습니다 · 맨 위로'가 6초 뜬다.
