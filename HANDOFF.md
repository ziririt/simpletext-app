...
- 현재 버전: **3.17.9+231** (`pubspec.yaml`, `lib/version.dart`)
...
- 메모를 열고 전환이 끝나면 머리 밑에 '책갈피 62%에서 이어 읽기' 알림이
  6초 뜬다. **자동으로 데려가지 않는다** — 다시 여는 까닭이 늘 이어 읽기는
  아니고, 묻지도 않고 3천 픽셀 아래로 던지는 것은 되찾아 주는 것이 아니라
  뺏는 것이다. 권하기만 하고 스스로 물러난다.
