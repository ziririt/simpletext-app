...
- 현재 버전: **3.17.18+240** (`pubspec.yaml`, `lib/version.dart`)
- App Store 마케팅 버전: **1.6 — 2026-09-10 승인·출시됨(READY_FOR_SALE), 붙은 빌드 240.**
  225 → 230 → 234 → 237 → **240** 으로 네 번 갈아 끼웠다(§4.1-4, §4.1-5).
  마지막 교체(237 → 240)는 IN_REVIEW 를 빼고 한 것이라 값을 치렀다 — §4.1-5.
  내용: 안드로이드 문구 정리 + 화면 전환 + 광고 구조 + 스크롤 책갈피 + **보기 설정**
  + 돋보기 유령 + 왼손 핸들 + 본문 하단 광고 복구
  1.5는 출시됐지만 **옛 코드가 나갔다**(§4.1-2). 1.6이 그것을 바로잡는 판이다
  (앱 버전 3.17.18과 다른 계통이다. 헷갈리지 말 것)
